...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    console.log('tailles - etude:', (data.etude_pdf_b64||'').length, 'docx:', (data.docx_b64||'').length);
        ...
        // PDF devis — utiliser le PDF jsPDF envoyé par le navigateur
    let devisPdfB64 = (data.devis_pdf_b64 && data.devis_pdf_b64.length > 100) ? data.devis_pdf_b64 : null;
    console.log('devis_pdf_b64:', devisPdfB64 ? devisPdfB64.length : 'absent');
    ...
    const htmlAdmin = '<!DOCTYPE html><html><head><meta charset="UTF-8"></head>'
      ...
      + '<p>' + (data.tarif_nom||'') + ' — ' + (data.tarif_off||'') + ' : ' + (data.facture_an||'') + ' EUR/an</p>'
    ...
    // Client : étude PDF + devis PDF (converti depuis Word)
    ...
    // Admin : étude PDF + devis PDF + devis Word
