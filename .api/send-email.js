...
// Reçoit les données du configurateur et envoie l'email via Brevo

export default async function handler(req, res) {

  // CORS — autoriser ton domaine GitHub Pages + ton futur site
  ...
  try {
    ...
    const lignesHtml = (data.lignes_devis || []).map(l =>
      `<tr><td>${l.l}</td><td style="text-align:right;font-weight:600;">${l.v} EUR HT</td></tr>`
    ).join('');

    const classementHtml = (data.classement || []).map((r, i) => {
      const bg = i === 0 ? 'background:#e8fff4;font-weight:700;' : (i % 2 === 0 ? 'background:#f8f8f8;' : '');
      const dc = i === 0 ? '#00875a' : '#555';
      const difc = String(r.diff || '').startsWith('-') ? '#00875a' : '#c0392b';
      return `<tr style="${bg}">
        <td style="color:${dc};">${i === 0 ? '*' : i + 1}</td>
        <td>${r.n}</td><td>${r.o}</td>
        <td style="text-align:right;">${r.ft} EUR</td>
        <td style="text-align:right;color:${difc};">${r.diff} EUR</td>
      </tr>`;
    }).join('');
...
.tot{font-size:14px;font-weight:800;color:#00875a;}
...
<div class="bloc"><h2>Coordonnees client</h2>
  ...
  ${data.msg ? `<div class="row"><span class="lbl">Message</span><span class="val">${data.msg}</span></div>` : ''}
</div>
...
<div class="bloc"><h2>Devis borne retenue</h2>
  ...
  <div class="row"><span class="lbl">Puissance</span><span class="val">${data.borne_kw} kW</span></div>
  <div class="row"><span class="lbl">Nb bornes</span><span class="val">${data.nb_bornes}</span></div>
  <table><tbody>
    ${lignesHtml}
    ...
    ${Number(data.devis_adv) > 0 ? `<tr style="font-weight:800;color:#00875a;"><td>Net apres prime ADVENIR</td><td style="text-align:right;">${data.devis_net} EUR</td></tr>` : ''}
  </tbody></table>
</div>

<div class="bloc"><h2>Recommandation tarifaire</h2>
  <div class="row"><span class="lbl">Fournisseur</span><span class="val">${data.tarif_nom}</span></div>
  <div class="row"><span class="lbl">Offre</span><span class="val">${data.tarif_off}</span></div>
  ...
  <div class="row"><span class="lbl">ROI</span><span class="val">${data.roi}</span></div>
  <div class="row"><span class="lbl">Cout 100km VE</span><span class="val">${data.c100km} EUR</span></div>
</div>
...
<div class="bloc"><h2>Classement des offres</h2>
  ...
  <tbody>${classementHtml}</tbody></table>
</div>
...
<div class="ft">IRVE Studio - Devis estimatif valable 30 jours - ${data.date_etude}</div>
    ...
    if (data.pdf_b64) {
      attachments.push({
        name: `etude-irve-${safeName}-${safeDate}.pdf`,
        content: data.pdf_b64,
      });
    }
    if (data.docx_b64) {
      attachments.push({
        name: `devis-irve-${safeName}-${safeDate}.docx`,
        content: data.docx_b64,
      });
    }
