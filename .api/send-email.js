...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!BREVO_KEY || !MON_EMAIL) {
      return res.status(500).json({ error: 'Variables manquantes : BREVO_API_KEY et MON_EMAIL requis' });
    }
    ...
    const classHtml = (data.classement || []).map((r, i) => {
...
<div class="hd">
  <h1>Nouvelle etude IRVE - ${data.nom}</h1>
  <p>Recue le ${data.date_etude} via IRVE Studio</p>
</div>
...
<div class="bloc"><h2>Classement des offres</h2>
  <table><thead><tr>
    <th>#</th><th>Fournisseur</th><th>Offre</th><th>Facture/an</th><th>Diff.</th>
  </tr></thead><tbody>${classHtml}</tbody></table>
</div>
